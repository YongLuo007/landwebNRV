...
setMethod(
  ...
  definition = function(lgptreeRaw,
    ...
    lgpHeader <- lgpHeaderRaw[nfi_plot %in% unique(lgptreeRaw$nfi_plot),][
    ...
    lgpHeader <- lgpHeader[,.(MeasureID, OrigPlotID1, MeasureYear, Longitude,
                            Latitude, Zone = NA, Easting = NA, Northing = NA, PlotSize,
                            baseYear, baseSA)]
    return(list(plotHeaderData = lgpHeader,
                treeData = treeData))
  })
